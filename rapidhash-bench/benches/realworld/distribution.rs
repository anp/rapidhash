...
new_distribution!(U32, u32, rng, rng.random::<u32>() | 1, rng.random::<u32>() & !1);
new_distribution!(U64, u64, rng, rng.random::<u64>() | 1, rng.random::<u64>() & !1);

new_distribution!(
    U64HiBits,
    u64,
    rng,
    ((rng.random::<u16>() as u64) << 48) | 1,
    (rng.random::<u16>() as u64) << 48
);
...
new_distribution!(
    ...
    u64,
    rng,
    (rng.random::<u16>() as u64) | (1 << 63),
    rng.random::<u16>() as u64
);
...
new_distribution!(
    ...
    (u8, u8, u8, u8),
    rng,
    (rng.random(), rng.random(), rng.random(), rng.random::<u8>() | 1),
    (rng.random(), rng.random(), rng.random(), rng.random::<u8>() & !1)
);
...
new_distribution!(
    ...
    Ipv4Addr,
    rng,
    rng.random::<[u8; 4]>().map(|c| c | 1).into(),
    rng.random::<[u8; 4]>().map(|c| c & !1).into()
);
...
new_distribution!(
    ...
    Ipv6Addr,
    rng,
    rng.random::<[u8; 16]>().map(|c| c | 1).into(),
    rng.random::<[u8; 16]>().map(|c| c & !1).into()
);
...
new_distribution!(
    ...
    String,
    rng,
    Uuid::from_u128(rng.random::<u128>() | 1).to_string(),
    Uuid::from_u128(rng.random::<u128>() & !1).to_string()
);
...
new_distribution!(
    ...
    Vec<u8>,
    rng,
    (0..1024).map(|_| rng.random::<u8>() | 1).collect(),
    (0..1024).map(|_| rng.random::<u8>() & !1).collect()
);
...
new_distribution!(
    ...
    Vec<u8>,
    rng,
    (0..1024 * 10).map(|_| rng.random::<u8>() | 1).collect(),
    (0..1024 * 10).map(|_| rng.random::<u8>() & !1).collect()
);
...
impl Distribution for AccessLog {
    ...
    fn sample<R: Rng>(&mut self, rng: &mut R) -> Self::Value {
        let resource_id = rng.random();
        let user_id = rng.random::<u32>() | 1;
        let date =
            chrono::NaiveDate::from_num_days_from_ce_opt(rng.random_range(0..365 * 100)).unwrap();
        let success = rng.random();
